...
# /clock.txt, `init 0` flushes it, efsread.py lifts it off the image. The host's
...
rep "core: $(rsh "md5sum /media/fat/$MISTER_CORE_FOLDER/$RBF_REMOTE" | cut -c1-32)"
...
sleep "$GAP"
say "second reading"
ws "text:date >> /clock.txt; cat /etc/TIMEZONE >> /clock.txt"
sleep 1
...
rsh "sleep 75"
rep "---- /clock.txt from the image ----"
rsh "cd $DBG; python3 efsread.py '$IMG' cat /clock.txt" 2>&1 | tee -a "$OUT"
say "report -> $OUT"
