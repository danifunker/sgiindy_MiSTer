...
- `--no-icache` / `--no-dcache`, which run with one or both primary caches off.
  ...
  rather than by rebuilding, and the four-way table in `docs/reference/cpu.md` is what they
  measure;
...
- **the PC**, from `dbg_pc` in `rtl/cpu/r4300/cpu.vhd`. The last 64 decoded
  ...
  run — see `docs/reference/cpu-validation.md`, "A TLB refill taken with EXL set". Read the cycle
...
- **`--pc-user FILE`**, every user-mode PC with its cycle, for diffing two runs
  ...
  retire-accurate trace and are **not finished** — see `docs/reference/cpu-validation.md`;
- `--irq`, one line per change of INT2's five lines into the CPU, with the
  ...
  the SCSI chip instead, which retired a wrong diagnosis in `docs/reference/chipset.md`;
...
[09-cpu-validation.md](cpu-validation.md).
