...
[10-r4300-integration.md](cpu.md). The plan it followed:
...
2. **`--irq`.** It showed the kernel setting `L0_MASK = 0x82`, `L1_MASK =
   ...
   both were wrong.
3. **The PC.** `dbg_pc` did not exist; `docs/reference/simulation.md` had listed it as the one
...
`docs/reference/simulation.md` recommends diffing against MAME. IRIS is better on
...
FPU fixes in [10-r4300-integration.md](cpu.md) it passes
fifteen of them.
...
[11-running-on-hardware.md](cpu-tests-on-hardware.md). **Note that the suite is
