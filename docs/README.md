...
| **[26-resume-tlb-epc-fix.md](26-resume-tlb-epc-fix.md)** | **Mostly superseded**: the fix it was written to finish now works, and the attempt table records why six of the seven did not. What is still live in it is the **board validation** and the two independent defects - the SCSI MESSAGE IN failure, measured end to end, and the 4 MB memory-sizing discrepancy |
| **[25-lost-store-tlb-order.md](25-lost-store-tlb-order.md)** | **The lost store: found, proven, FIXED and verified on hardware.** `init died` is gone in both simulator configurations and on the board three deploys out of three; IRIX 5.3 now banners and runs `fsck`, and the next wall is SCSI. It was never in the memory path. `init` stores its table pointer in the delay slot of a `jal`; the store's page and the branch target's page miss the TLB together; the fetch stage runs ahead of execute and an instruction in EXECUTE cannot even ask for its translation while the fetch is stalled, so the *younger* instruction's exception is always taken first, sets EXL, and the store's own fault arrives "nested" with its EPC write suppressed. The kernel returns to the branch target and the delay slot never runs again. Not the arbiter, which was tried and changed nothing; the fix is one term on the EXL guard, plus deleting a per-clock default that had been quietly turning the flag qualifying it into a one-cycle pulse |
| **[27-multiuser-on-hardware.md](27-multiuser-on-hardware.md)** | **IRIX 5.3 reaches run level 2 on the board.** What proves it, why the disk image was a confound (the board had been running a dirty one), and the SCSI wedge that is real, measured and STILL OPEN - with the five hypotheses already ruled out, the watchdogs that exist for the next attempt, and the next walls: Newport pixel DMA, the CD-ROM target, and no Ethernet. Also: how to boot single-user |
| **[28-resume-scsi-wedge.md](28-resume-scsi-wedge.md)** | **The current work item**: `fsck` wedges the SCSI subsystem permanently, so a board that reaches multiuser cannot survive its own reset. The core stops asking the HPS for blocks entirely - measured - so it is in the SCSI RTL. Eight hypotheses ruled out with the evidence, `io_ack` left standing, a 5-minute board reproducer and the five watchdogs to point at it |
