...
Written 2026-09-17, after docs/53. docs/52 §7 listed `sgi_hpc3`'s arrays as the
next flip-flop storage in the core; this is that change, and it is the third of
the same shape (`eeprom_93c56` and `np_bt445` in build 37, docs/52 §2).
...
The device is 81 % full where docs/50's speed builds were in the mid-nineties,
