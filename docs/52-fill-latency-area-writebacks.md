...
Written 2026-09-16/17, resuming docs/51's list of what was left: line fills
...
docs/50 §4 measured L - "the bridge answers a read's first word 9.7 clocks
...
CPU read cost it well under a clock since FBR_SUB went to 4 (docs/51 §11); the
...
LOAD_NO_STALL (docs/51 §6) let a load leave execute without holding it only
