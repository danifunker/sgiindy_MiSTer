...
//
//  WHAT IS CHECKED. Every pixel of every frame after the first is the byte
...
int main(int argc, char **argv)
{
    ...
    // A frame buffer's worth of distinct values.
    for (int y = 0; y < V_VIS; y++)
        for (int x = 0; x < H_VIS; x++) {
            uint32_t a = ((uint32_t)y * STRIDE + x) * 8;
            mem[a] = cell(a);
        }
    ...
    const int FRAMES = 3;
    uint64_t checked = 0, wrong = 0, misses[FRAMES] = {0};
    uint64_t shown = 0;
    int frame_now = 0;
    ...
    struct Expect { uint32_t addr; bool visible; int frame; };
    ...
    auto sample = [&]() {
        ...
        if (!e.visible) return;
        checked++;
        uint64_t want = mem[e.addr];
        if (dut->px_rdata != want) {
            wrong++;
            if (shown < 8) {
                printf("  FAILED frame %d addr %08x: want %016llx got %016llx\n",
                       e.frame, e.addr, (unsigned long long)want,
    ...
    for (int f = 0; f < FRAMES; f++) {
        ...
        for (int y = 0; y < V_TOTAL; y++) {
            ...
            for (int x = 0; x < H_TOTAL; x++) {
                ...
                dut->px_addr = visible ? a : ((uint32_t)y * STRIDE) * 8;
                dut->px_req = visible;
                if (visible) inflight_px.push_back({a, true, f});
                tick();
                sample();
                dut->px_req = 0;
        ...
        printf("frame %d: %llu misses\n", f, (unsigned long long)misses[f]);
    }

    printf("\n%llu pixels checked over %d frames, %llu clocks\n",
    ...
    check("every pixel came back as the byte that was in memory", wrong == 0);
